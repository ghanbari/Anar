...
include dirname(__FILE__) . DS . '..' . DS . '..' . DS . '..' . DS . '..' . DS . '..' . DS . 'vendor' . DS . 'autoload.php';

$session = new Session();
$session->start();

$permissions = $session->get($session->get('blogName') . '/fileManager/permission');
...
function directorySize($path)
{
    ...
    foreach ($files as $t)
    {
        if ($t != "." && $t != "..")
        {
            ...
            {
                $size = directorySize($currentFile);
                $total_size += $size;
            }
            else
            {
...
$config = array(
    ...
    */
    ...
    |
    */
    'upload_dir' => '/uploads/' . $session->get('blogName') . '/',
    ...
    |
    */
    'current_path' => '../../../uploads/' . $session->get('blogName') . '/',
    ...
    |
    */
    'thumbs_base_path' => '../../../images/thumb/' . $session->get('blogName') . '/',
    ...
    */
    ...
    'access_keys' => $session->has($session->get('blogName') . '/fileManager/accessKey')
        ? array($session->get($session->get('blogName') . '/fileManager/accessKey')) : array(),
    ...
    |
    */
    'MaxSizeUpload' => $session->has($session->get('blogName').'/fileManager/maxUploadSize') ? $session->get($session->get('blogName').'/fileManager/maxUploadSize') : 0,
    ...
    */
     ...
     *******************/
    'aviary_active' => true,
    'aviary_apiKey' => "206998a3b839ff9e",
    'aviary_secret' => "8d8549affad9096c",
...
$driveSize = directorySize(__DIR__ . DS . '..' . DS . $config['current_path']) / 1048576; //Mb
$remainSize = $session->get($session->get('blogName') . '/fileManager/maxDriveSize') - $driveSize;

if (($remainSize < 0) or (($remainSize - $config['MaxSizeUpload']) < 0)) {
    $config['MaxSizeUpload'] = abs($remainSize);
}
